...
x_train, x_test, y_train, y_test, input_reference, output_reference = get_pump_data(undulators_2_datasets[1])
...
np.savez("pumpProbe2021/results/ex_1_pump_pred/ann_pred.npz",
...
for l in range(len(ranking)):
    ...
    x_te_masked = np.stack(x_te_masked).T
    scores.append(ann.evaluate(x_te_masked, y_test)[1]*output_reference.loc['test_std', 'vls_com_pump'])
...
key_features = i_ref.columns[ranking][:20]
key_feat_ind = ranking[:20]
...
np.savez("pumpProbe2021/results/ex_1_pump_pred/ann_10_feat_pred.npz",
         train_out=train_out, train_pred=train_pred, test_out=test_out, test_pred=test_pred)

#%%

label = f"ANN; MAE: {round(new_ann.evaluate(x_te_filt, y_test)[1]*output_reference.loc['test_std', 'vls_com_pump'], 2)}"

plot_fit.plot_pvm(test_out, test_pred,
                  label,
                  "Expected Pump CoM (eV)", "Predicted Pump CoM (eV)",
                  "pumpProbe2021/results/ex_1_pump_pred/ann_low_pump_hist2d")
