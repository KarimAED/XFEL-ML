...
x_train, x_test, y_train, y_test, inp_df, out_df = get_probe_data(undulators_2_datasets[1], include_pump=False)
...
test_std = output_reference.loc['test_std', 'vls_com_probe']
...
plot_fit.plot_pvm(y_test, new_xgb.predict(x_te_filt),
                  label,
                  "Expected Probe CoM", "Predicted Probe CoM",
                  "pumpProbe2021/results/ex_2_pump_probe_corr/xgb_feat_probe_hist2d_no_corr")

#%%
predictions = new_xgb.predict(x_te_filt)

out_ref = output_reference["vls_com_probe"]
...
np.savez("pumpProbe2021/results/ex_2_pump_probe_corr/xgb_10_feat_probe_pred.npz",
