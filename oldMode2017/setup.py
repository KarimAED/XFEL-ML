"""
Setup file used to define the data loading pipelines
for the old mode data from 2017.
"""
import os
import logging
...
def format_inp():
    """
    Function to collect all data used for input features
    and format it properly.

    :return: tuple of 2d-array + 1d-array,
        corresponding to feature data & names respectively
    """
    ...
    for f_name in files:
        if any(
            (i in f_name for i in ["EBeam", "EPICS", "GMD"])
        ):  # avoid unnecessary data loading
            data = np.load(os.path.join(source, f_name))
        if "EBeam" in f_name:
            if not ebeam_labels:
                ebeam_data = data["EBeamValuesList"]
                ebeam_labels = data["EBeamParameterNames"].tolist()
            else:
                ebeam_data = np.append(
                    ebeam_data, data["EBeamValuesList"], axis=0
                )
        elif "EPICS" in f_name:
            if not epic_labels:
                epic_data = data["EPICSValuesList"]
                epic_labels = data["EPICSParameterNames"].tolist()
            else:
                epic_data = np.append(
                    epic_data, data["EPICSValuesList"], axis=0
                )
        elif "GMD" in f_name:
            if not gmd_labels:
...
def format_outp():
    """
    Function to collect all data used for
    output labels and format it properly.

    :return: tuple of 2d-array + 1d-array,
        corresponding to label data & names respectively
    """
    ...
    for f_name in files:
        if "Delay" in f_name:
            ...
            if not delay_labels:
                ...
                delay_data = np.array(
                    [
                        data["DelayValuesList"].flatten(),
                        data["DelayValuesListMask"],
                    ]
                ).T
            else:
                temp_data = np.array(
...
def get_data(filter_by_corr=False, filter_cols=None):
    """
    Function to collect both input and output data
    for oldMode2017, and format the data.

    :param filter_by_corr: bool, if highly correlated
        input features should be removed
    :param filter_cols: array-like of str, manually select
        only a few columns to be included
    :return: array-like, contains x_train, x_test, y_train,
        y_test, input_reference, output_reference
    """
    ...
    arg_mask = np.argwhere(
        mad_mask & emask
    ).flatten()  # generate yet another arg_mask
    ...
    var_thresh = 10
    feat_columns = [
        c for c in delay_inp if len(np.unique(delay_inp[c])) > var_thresh
    ]
    ...
    if filter_by_corr:
        ...
        # We convert the correlation matrix
        # to a distance matrix before performing
        # hierarchical clustering using Ward's linkage.
        distance_matrix = 1 - np.abs(corr)
        dist_linkage = hierarchy.ward(squareform(distance_matrix))
        dendro = hierarchy.dendrogram(
            dist_linkage,
            labels=list(range(len(delay_inp.columns))),
            leaf_rotation=90,
        )
        _ = np.arange(0, len(dendro["ivl"]))
        cluster_ids = hierarchy.fcluster(
            dist_linkage, 0.7, criterion="distance"
        )
