...
def lin_feature_pipeline(
    data, string_data, pred_lims=False, legend=True, vmax=None, noRefit=False
):
    x_train, x_test, y_train, y_test, input_reference, output_reference = data

    lin_model = lin_pipeline(
        data, string_data, plot=False, save=False
    )  # if we don't plot, no need to pass other kwarg
    ...
    for l in range(0, len(ranking), 5):
        ...
        scores.append(
            helpers.mae(temp_lin.predict(data_temp[1]), y_test)
